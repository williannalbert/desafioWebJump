...
$dado = $prodSql->getOne()[0];
$categorias = $prodSql->getProdutosCategorias($_GET['sku']);
$todasCategorias = new categoriesSql();
?>
<!doctype html>
<html ⚡>
<head>
...
<!-- Header -->
  ...
  <main class="content">
    ...
    <form action="..\productsController.php?sku=<?php echo $dado['sku']?>" method="post" enctype="multipart/form-data">
      <div class="input-field">
        <label for="sku" class="label">Product SKU</label>
        <input type="text" id="sku" class="input-text" name="sku" value=<?php echo $dado['sku'] ?>> 
      </div>
      <div class="input-field">
        <label for="name" class="label">Product Name</label>
        <input type="text" id="name" class="input-text" name="name" value=<?php echo $dado['nome'] ?>> 
      </div>
      <div class="input-field">
        <label for="price" class="label">Price</label>
        <input type="text" id="price" class="input-text" name="price" value=<?php echo $dado['preco'] ?>> 
      </div>
      <div class="input-field">
        <label for="quantity" class="label">Quantity</label>
        <input type="text" id="quantity" class="input-text" name="quantity" value=<?php echo $dado['quantidade'] ?>> 
      </div>
      <div class="input-field">
        <label for="category" class="label">Categories</label>
        <select multiple id="category" class="input-text" name="category">
          <?php
          foreach($todasCategorias->getAll() as $key => $item){
      ...
      <div class="input-field">
        ...
        <textarea id="description" class="input-text" name="description"><?php echo $dado['descricao'] ?></textarea>
      </div>
        <?php if(!empty($dado['imagem'])){
          echo '<div class="input-field">
                  <center><img src="..\imgs/'.$dado['imagem'].'" width="100px height="100px"></center>
                </div>';
        }
        ?>
      <div class="input-field">
      ...
      <div class="actions-form">
        ...
        <input class="btn-submit btn-action" type="submit" value="Save Product" name="edit"/>
      </div>
